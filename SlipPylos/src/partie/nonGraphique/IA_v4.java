...
public class IA_v4 {
	...
	public double playOnce_inst(PylosPartie partie, TeamType monEquipe, int profondeurDeRecherche, boolean playForReal, boolean evaluateMax) {
		...
		for (int iCell = 0; iCell < playableCells.size(); iCell++) {
			...
			poseJeton_noCheck(caseEvaluee.hauteur, caseEvaluee.xCell, caseEvaluee.yCell, equipeQuiJoue); // incr�mente les compteurs de points
			double score = playOnce_inst(null /* pour �tre s�r de ne rien modifier */, monEquipe, profondeurDeRecherche - 1, false, !evaluateMax);
