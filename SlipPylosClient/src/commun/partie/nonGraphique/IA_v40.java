...
public class IA_v40 {
	...
	public static ArrayList<IA_v40_cell> getPlayableCells(PylosGridArray plateauActuel) {
		ArrayList<IA_v40_cell> result = new ArrayList<IA_v40_cell>();
		for (int iGrid = 0; iGrid <= plateauActuel.getHauteurMax(); iGrid++) { // toutes les grilles
			...
			for (int xCell = 0; xCell < grid.gridWidth; xCell++) { // toutes les cases en x
				for (int yCell = 0; yCell < grid.gridHeight; yCell++) { // toutes les cases en y
					if (grid.canPlaceAtPosition(xCell, yCell)) { // si je peux placer un pion à cette position, je place
						IA_v40_cell cell = new IA_v40_cell(xCell, yCell, iGrid, grid);
						result.add(cell);
					}
				}
			}
	...
	public double playOnce_inst(PylosPartie partie, TeamType monEquipe, int profondeurDeRecherche, boolean playForReal, boolean evaluateMax) {
		if (playForReal) {
			...
			plateauActuel = partie.plateauActuel.copy(partie); // pour ne pas embêter l'affichage graphique
		} else {
		...
		IA_v40_cell caseRetenue = null;
		
		// C'est à moi de jouer : Je prends le score maximal des cases évaluées cette itération
		ArrayList<IA_v40_cell> playableCells = getPlayableCells(plateauActuel);
		ArrayList<IA_v40_cell> sameScoreCells = new ArrayList<IA_v40_cell>();
		...
		for (int iCell = 0; iCell < playableCells.size(); iCell++) {
			...
			IA_v40_cell caseEvaluee = playableCells.get(iCell);
		...
		if (caseRetenue == null) {
			Log.write("ERREUR IA_v40.playOnce_inst : caseRetenue == null");
			return 0;
		}
		...
		if (size == 0) {
			Log.write("ERREUR IA_v40.playOnce_inst : size == 0");
			return 0;
		}
		...
		IA_v40_cell caseRetenue_alt = caseRetenue;
		caseRetenue = sameScoreCells.get(choosenIndex);
		//Log.write("IA_v40.playOnce_inst : choosenIndex = " + choosenIndex);
