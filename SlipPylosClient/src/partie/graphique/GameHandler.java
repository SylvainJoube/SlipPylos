...
import partie.nonGraphique.PylosCellResult;
import partie.nonGraphique.PylosPartie;
import partie.nonGraphique.PylosPoint;
import partie.nonGraphique.*;
	...
	@Override public void mousePressed(MouseEvent event) {
		...
		if (GameHandler.tourSuivantPos.isInside(event.getX(), event.getY())) {
			if (!partie.joueurAUtiliseSaReserve) {
				LogWriter.Log("MyMouseListener.mousePressed : Impossible de passer son tour lorsque vous n'avez pas encore jou� !");
		...
		while (gameRunning) {
			...
			if (currentCellUnderMouse != null) {
				...
				if (!currentCellUnderMouse.peutPoserIci)
					g.drawImage(jetonErreurImg, xDraw, yDraw, null);
